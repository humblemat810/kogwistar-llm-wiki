...
The lane service result/lookup DTOs (`LaneMessageSendResult`,
`LaneMessageProjectionRepairResult`, and `LaneMessageLookup`) are now slotted
as well; they have fixed fields and are returned on the same messaging paths.
...
The next core scan found three small context/result records with repeated
construction sites and no dynamic layout contract: `conversation.ContextMessage`,
`conversation.DroppedItem`, and `conversation.RetrievalOutcome`. They are now
strictly slotted and covered by `tests/core/test_high_cardinality_layout.py`.
The root benchmark records their before/after layout and construction metrics
with the other migrated classes. `ContextItem` remains intentionally deferred
because the context packing path currently copies it through `it.__dict__`.
