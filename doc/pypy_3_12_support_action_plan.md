...
- optional MCP transport and telemetry dependency trees;
...
| Experimental | PyPy 3.11 release | Run Python-authority provider-free probes using a pinned release build; native extensions remain a separate gate. |
...
## PyPy 3.11 Experimental Lane

PyPy 3.11 is tested separately from the PyPy 3.12 beta lane. The workflow
uses the immutable `pypy3.11-v7.3.20-linux64.tar.bz2` release archive and
verifies its SHA-256 before extraction. It is automatic and nonblocking so a
dependency or interpreter regression cannot block CPython releases.

The initial 3.11 lane is intentionally a Python-authority control-plane
probe. It excludes NumPy, Chroma, pgvector, pikepdf, Torch, and GPU packages,
sets `KOGWISTAR_IMPL_MODE=python`, and runs the provider-free CI selection.
The Kogwistar native extension is not silently treated as compatible, and no
PyPy image is published from this lane. Native extension loading, PostgreSQL
vector storage, and a production image require separate passing gates and a
deliberate support decision.

The lane uses `--ignore-requires-python` only to probe the existing source
against Python 3.11. This does not change package metadata: the application,
Kogwistar, and parser still declare Python 3.12 as their production floor.

The tested application profile is Linux/WSL-oriented. A native Windows PyPy
3.11 interpreter can be provisioned, but the MCP dependency path conditionally
requires `pywin32` only on Windows and currently has no matching PyPy
distribution. This is a Windows development-profile limitation, not a Linux
container-image blocker: the Linux gate runs on `ubuntu-latest` and does not
install `pywin32`. Native Windows PyPy must not be promoted by replacing that
dependency with an untested compatibility package. Native Windows slot
measurements may use a dependency-light benchmark; Linux image eligibility is
decided only by the Linux interpreter, dependency, application, and container
smoke gates.

The Linux workflow installs the application with `--no-deps` and places the
three sibling source checkouts on `PYTHONPATH` so the provider-free probe does
not build their native packages. Its `pip check` output is therefore recorded
as resolver evidence but is informational when it reports those source
packages as not installed. The legacy standalone FastMCP package is
intentionally excluded; the current implementation uses the official `mcp`
SDK through the same adapter on CPython and PyPy.

The current Linux provider-free gate is green in WSL and in the pinned CI
container. The container smoke workflow creates a PyPy 3.11 virtual environment
inside the pinned image and runs the same full provider-free application test
selection, including the official MCP gateway tests, with timing output. The
shared MCP adapter has now been migrated to the official `mcp`
SDK, removing the legacy FastMCP dependency and its PyPy-specific candidate
lane. The full MCP-enabled application gate still requires a clean PyPy 3.11
protocol smoke test before image publication. A passing
candidate must still pass the complete provider-free application suite,
native-extension gate, and container health smoke before an image publication
workflow may be added.

The separate `slots-benchmark` workflow measures the same application-owned
object layouts on CPython 3.12, CPython 3.13, CPython 3.14, and PyPy 3.11. It
uploads JSON artifacts containing wall time, process CPU time, CPU utilization,
and RSS where available. The PyPy run uses a warm-up population. Optional
per-test pytest resource reports use the same wall/CPU/CPU-utilization/RSS
fields; traced allocation is an explicit slower mode because some PyPy builds
do not provide `_tracemalloc`.
These measurements are nonblocking evidence only; they do not establish
native extension compatibility and do not authorize a PyPy image publication.
...
| --- | --- | --- | --- | --- |
| Pydantic/pydantic-core | all profiles | probe | The candidate pair `pydantic==2.12.5` / `pydantic-core==2.41.5` builds and installs, but the binary audit reports missing `PyList_GET_ITEM`, `PyList_GET_SIZE`, `PyList_SET_ITEM`, `PyObject_LengthHint`, `PyPyFrozenSet_Check`, `PyPySet_Check`, `_PyLong_AsByteArray`, `_PyLong_FromByteArray`, `_PyLong_NumBits`, and `_Py_Dealloc`; `RTLD_NOW` confirms the loader failure at `PyList_GET_SIZE`. | blocking; upstream/native PyO3 FFI fix or a separately validated CI-only patch required |
| Kogwistar PyO3 extension | all graph profiles | probe | The independently built `_rust` extension fails the binary/loader gate with `PyPyExc_AttributeError` missing from the PyPy 3.12 runtime. | blocking |
| Official MCP SDK | MCP profile | probe | probe | required before PyPy promotion |
...
The LLM-Wiki repository provides an automatic, non-blocking application probe
at `.github/workflows/pypy-beta.yml`, with a manual dispatch option for an
installed-wheel run. It runs on pull requests, pushes to `main`, and weekly;
the reusable local checker is:
...
The application workflow remains non-required by design. It records the exact
interpreter and wheel-tag diagnostics, but does not claim production support.
Each PyPy probe also uploads a JSON profile report plus `pip freeze` and
`pip check` output, including when the crash-sensitive import check fails.
The optional installed-wheel probe uploads its own profile result so resolver
and import failures remain inspectable after the job terminates.
Kogwistar core now also runs an automatic, non-blocking
`pypy-beta-best-effort` job in its normal `.github/workflows/ci.yml`; that job
builds the PyO3 extension from source, enumerates the exact Python ABI symbols
required by each finished native extension with `nm`, compares them with the
exports of the exact PyPy runtime, runs an `RTLD_NOW` loader preflight, and
only then attempts direct extension/package imports and provider-free tests.
The PyO3 source scan remains supplementary diagnostics, not compatibility
evidence. A failed beta probe remains visible without blocking required CPython
or Rust jobs. The current evidence proves wheel build/install progress but
does not yet prove the pydantic-core or Kogwistar Rust ABI gates; keep the
profile blocked until both independent native reports and the selected
provider-free tests pass together.
The best-effort core job uses the immutable PyPy v8.0.0 beta archive
`pypy3.12-v8.0.0-linux64.tar.gz` and verifies its published SHA-256 before
extraction. A manual dispatch can override both the archive URL and checksum
when another versioned beta is deliberately evaluated; the log prints expected
and actual hashes so checksum failures are actionable.
...
The application lane uses the same immutable archive and checksum defaults,
with manual URL/checksum overrides for deliberate beta refreshes. It remains a
non-required capability probe because the interpreter and native dependency
ecosystem are still experimental; do not promote it to a required release gate.
...
`Dockerfile.pypy-ci`, requires a versioned archive URL, SHA-256, and explicit
`PYPY_VERSION` build argument, and is
...
PyPy archive checksum and exact build remain explicit workflow inputs even
though the default is now the official v8.0.0 beta release.

The scheduled/manual `pypy-311-ci-container.yml` workflow uses the same
Dockerfile with `PYPY_VERSION=3.11`. It builds and smoke-tests a provider-free
diagnostic image only; it does not publish a production application image or
claim native-extension/storage parity.
...
- [x] Pin merged Kogwistar, parser, and sink revisions in LLM-Wiki.
