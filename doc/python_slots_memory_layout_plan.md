...
The next high-cardinality core-record wave is also implemented with strict
slots for `ContextMessage`, `DroppedItem`, and `RetrievalOutcome`. These records
are repeatedly created while assembling conversation context or tool results;
they do not expose supported dynamic attributes or framework-managed layout.
The benchmark includes them alongside the application records. The Kogwistar
lane-messaging DTO wave additionally slots `LaneMessageSendResult`,
`LaneMessageProjectionRepairResult`, and `LaneMessageLookup`; these fixed
records are returned by the lane service and retain their dataclass copy
contract without exposing dynamic attributes.
...
- any class generated or wrapped by the MCP SDK, OpenTelemetry, Torch, or
  Transformers.
...
7. Profile a real bounded ingest/maintenance fixture to identify retained
   service instances and context-record populations.
...
| --- | ---: | ---: | ---: |
| `CodexBridgeState` | 5.997 -> 4.092 | 0.062500 -> 0.046875 s | 16.6% |
| `CodexMemoryService` | 4.095 -> 19.368 | 0.046875 -> 0.187500 s | 58.3% |
| `ComposeOptions` | 5.100 -> 4.815 | 0.046875 -> 0.046875 s | 24.9% |
| `ContextMessage` | 1.802 -> 2.191 | 0.015625 -> 0.031250 s | 35.5% |
| `DroppedItem` | 2.061 -> 1.604 | 0.031250 -> 0.015625 s | 35.5% |
| `LaneMessageLookup` | 5.573 -> 4.094 | 0.046875 -> 0.046875 s | 24.1% |
| `LaneMessageProjectionRepairResult` | 2.091 -> 2.901 | 0.015625 -> 0.031250 s | 33.2% |
| `LaneMessageSendResult` | 2.473 -> 2.920 | 0.015625 -> 0.031250 s | 33.2% |
| `LaunchStep` | 2.642 -> 2.846 | 0.031250 -> 0.031250 s | 35.5% |
| `LiveTracePrinter` | 5.007 -> 1.976 | 0.046875 -> 0.015625 s | 67.5% |
| `LlmWikiIdentity` | 3.066 -> 2.882 | 0.031250 -> 0.031250 s | 29.3% |
| `MaintenanceJobExecutionContext` | 2.370 -> 4.131 | 0.031250 -> 0.046875 s | 31.5% |
| `MaintenanceStrategyRegistry` | 5.230 -> 2.384 | 0.062500 -> 0.015625 s | 49.9% |
| `MappingAssetResolver` | 3.751 -> 1.510 | 0.031250 -> 0.015625 s | 48.0% |
| `RetrievalOutcome` | 4.111 -> 1.718 | 0.046875 -> 0.015625 s | 33.2% |
| `ReviewQueryService` | 3.783 -> 1.436 | 0.046875 -> 0.015625 s | 68.2% |
| `SemanticLensService` | 5.720 -> 2.219 | 0.062500 -> 0.031250 s | 61.1% |
| `TuiConfiguration` | 3.774 -> 4.462 | 0.031250 -> 0.046875 s | 24.9% |
| `WorkbenchInteractionStore` | 3.391 -> 0.862 | 0.031250 -> 0.015625 s | 68.2% |
...
The automatic `.github/workflows/slots-benchmark.yml` workflow repeats the
same benchmark on CPython 3.12, CPython 3.13, CPython 3.14, and the fixed
PyPy 3.11 release. It uploads one JSON artifact per interpreter. Each report
includes wall time, process CPU time, CPU utilization, and resident-memory
deltas where the runner exposes them; CPython also reports traced allocations.
The benchmark JSON uses `cpu_utilization_percent` for the derived CPU metric.
PyPy receives a
warm-up population so its JIT is not compared only during cold startup. These
jobs are deliberately nonblocking: runner scheduling noise makes them useful
for trend detection, not for a hard performance threshold or an image-release
decision. If the checked-out vendor revision predates the Kogwistar DTO slot
wave, the artifact still contains the application-owned measurements and lists
the unavailable vendor DTOs under the top-level `skipped` field with an
actionable pin-update reason. They are never reported as slotted until the
vendor revision is advanced and its own slot tests pass.
...
pypy3 scripts\benchmark_slots.py `
  ...
  --json-out doc\slots_benchmark_pypy311.json
```
